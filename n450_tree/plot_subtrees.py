...
subtrees = ['B3','D8']
for st in subtrees:
    tree_path = f"{st}/{st}_timetree/timetree.nexus"
    ...
    fig,ax = plt.subplots(figsize=(5,15),facecolor='w')
    ...
    all_countries=[tO.name.split('|')[1] if tO.is_leaf() else '' for tO in tree.Objects]
    ...
    for j,country in enumerate(countries):
        if country is not None:
            ...
            if country == 'SouthAfrica': #note, starting with a "5" indicates a private clinic derived sequence
    ...
    plt.savefig(f'{st}/{st}_time_resolved_tree.pdf',bbox_inches='tight',transparent=True)
    plt.close('all')
